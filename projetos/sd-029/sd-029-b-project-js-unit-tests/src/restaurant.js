...
const createMenu = (object) => {
  const obj = {
    ...
    pay: () => {
      let total = 0;
      obj.consumption.forEach((order) => {
        if (object.food[order]) {
          total += object.food[order];
        } else {
          total += object.drink[order];
        }
      });
      return total * 1.1;
    },
    order: (string) => {
      if (Object.prototype.hasOwnProperty.call(object.food, string)
      || Object.prototype.hasOwnProperty.call(object.drink, string)) {
