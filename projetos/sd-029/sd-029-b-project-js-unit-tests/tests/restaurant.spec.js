...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(createMenu()).toHaveProperty('fetchMenu', expect.any(Function));
    ...
    expect(Object.keys(menu.fetchMenu())).toHaveLength(2);
    ...
    expect(menu.order('picanha')).toEqual([]);
    expect(menu.order('coxinha')).toContain('coxinha');
    ...
    menu.order('sopa');
    menu.order('agua');
    expect(menu.consumption).toEqual(['coxinha', 'sopa', 'agua']);

    // 10: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    menu = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    menu.order('sopa');
    menu.order('sopa');
    expect(menu.consumption).toEqual(['sopa', 'sopa']);
