...
export function createCityElement(cityInfo) {
  ...
  button.innerHTML = 'Ver previsão';
  button.onclick = () => {
    return fetch(`http://api.weatherapi.com/v1/forecast.json?lang=pt&key=${import.meta.env.VITE_TOKEN}&q=${cityInfo.url}&days=7`)
      .then((response) => {
        return response.json().then((data) => {
          const forecast = data.forecast.forecastday.map((forecastDay) => Object.create(
            {
  ...
  searchCities(searchValue)
    .then((cities) => cities.forEach((city) => {
      getWeatherByCity(city.url)
        .then((weather) => document
          .getElementById('cities').appendChild(createCityElement(weather)));
    }));
}
